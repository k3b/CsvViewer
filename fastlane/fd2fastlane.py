...
def save_to_wiki_homepage(title, short_description, full_description, android_locale):
    if ((mylen(WIKI_OUT_ROOT) > 0)): # and (mylen(android_locale) > 0)):
        ...
        homepage_locale = translate_locale(android_locale, LANG_ANDROID_TO_WIKI)
        if (mylen(android_locale) > 0):
            filename =  'Home-' + homepage_locale + '.md'
        else:
            filename = homepage_locale + 'Home.md'
        ...
        if oldContent is None:
            ...
            save_to_file(md, WIKI_OUT_ROOT, filename, "save_to_wiki_homepage")


def generate_wiki_homepage(label, title, short_description, full_description):
    md = f"""{label}
    
<img src="CsvViewer.svg" width="80" height="80" >   
...
def save_to_app_about(title, short_description, full_description, android_locale):
    if (mylen(APP_OUT_RES_ROOT) > 0): # and (mylen(android_locale) > 0):
        ...
        if oldContent is None:
            ...
            # print(f'\t{full_md_path[-10:]}\t{oldContent[:10]}\t{mylen(oldContent)} {label}')
            save_to_file(md, dir, filename, "save_to_app_about")
...
<!--
/* 
 * This file is part of "CsvViewer" .
 *
...
def process_translation(fdroid_xml, android_locale):
    if os.path.isfile(fdroid_xml):
        ...
        save_to_file(title, cur_fastlane_out_dir, 'title.txt', "process_translation")
        save_to_file(short_description, cur_fastlane_out_dir, 'short_description.txt', "process_translation")
        ...
        if (full_description is not None):
            ...
            save_to_file(html, cur_fastlane_out_dir, 'full_description.txt', "process_translation (with full_description)")
...
def save_to_file(content, directory_path, filename, debug):
    if (content is not None):
        file_path = os.path.join(directory_path, filename)
        # print('\t' + file_path[-30:] + '\t' + content[:60])
        if not os.path.exists(directory_path):
            os.makedirs(directory_path)
        # print(f"{debug} Writing {file_path}...")
